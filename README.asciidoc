...
thread pairs sending and receiving packets over a single socket per pair.  All packets are sent to a single IPv4
or IPv6 address, and a single UDP port at that address (although see Send-to-Self Mode below).

Counts of packets sent and received, along with packet round-trip statistics, are collected and summarized on
...
tool chain, in either native or cross compilation configuration.
